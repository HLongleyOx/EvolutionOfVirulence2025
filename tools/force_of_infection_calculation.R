...
#      print(i)
    ...
    for (i in 1:num_strains){
      ...
      durations_temp = max_dur-365
      #}
#      print(length(spvl_i[(1:durations_temp),i]))
      spvl_mean <- weighted.mean(spvl_i[(1:durations_temp),i], w=seq(durations_temp,1,-1)*(1/(durations_temp)))
#      spvl_mean <- mean(spvl_i[(1:durations_temp),i])
      #spvl_mean <- spvl_i[1,i]
    ...
    for (i in 1:num_strains) {
      ...
      infect_i[(dur+1):time, i] = 0
      infect_i[(dur - AIDS_stage):dur,i] <- 0.76   
    }
    ...
    for (j in 1:num_strains) {
      for (i in 1:num_strains) {
        ...
        if (asymp_dur == 0) {
          KM2[i, j] = 0
          force_of_infection2[i, j, ] = 0
        } else {
          force_of_infection2[i, j, acute_stage:(asymp_dur-AIDS_stage)] = ((infect_i[, j] * muts_freq[i, j, ] *
                                                                             exp(-d * 0:(time -
                                                                                           1))))[acute_stage:(durations[j]-AIDS_stage)]
          f1 =  force_of_infection2[i, j, acute_stage:(asymp_dur-AIDS_stage)]
          KM2[i, j] <-  dt * trapz(y = f1, x = 1:(length(f1)))
        }
      }
    }
    
    #return(list(force_of_infection, durations_i, spvl_i, KM, durations))
    return(list(force_of_infection,durations_i, spvl_i, KM, durations, KM2))    
}
